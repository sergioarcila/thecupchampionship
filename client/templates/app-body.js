...
Template.appBody.onCreated( function(){
  this.documents = this.subscribe( 'matches' );
});
...
Template.appBody.helpers({
  ...
  getlastupdate: function(){  
    var y = Session.get('myDate'); HMS = y.getHours()*3600+y.getMinutes()*60+1*y.getSeconds(); 
    //return History.find({'time.year':y.getFullYear,'time.month':y.getMonth+1,'time.HMS':{$gt: HMS - 500}},{sort:{time:-1}})
    return History.find({'time.year':y.getFullYear(),'time.month':y.getMonth()+1,'time.HMS':{$gt: HMS - 10}},{sort:{time:-1}})
